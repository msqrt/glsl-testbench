...
float g(float x) {
	...
	const float sig = 8.;
	x = (x-mu)/sig;
	return 20./(sig*sqrt(2.*3.141592))*exp(-.5*x*x);
}
...
vec4 cubic(vec2 uv) {
	...
	for(int i = 0; i<4; ++i)
		for(int j = 0; j<4; ++j) {
			...
			if(pcoord.y>=size.y-1 && pcoord.x>=252 && pcoord.x<260)
				val = vec4(g(float(pcoord.x)));
			mins = min(val, mins);
			maxs = max(val, maxs);
...
vec4 cubicVel(vec2 uv) {
	...
	for(int i = 0; i<4; ++i)
		for(int j = 0; j<4; ++j) {
			...
			if(pcoord.x<1||pcoord.y<1||pcoord.x>=size.x-1||pcoord.y>=size.y) val = vec4(.0);
			if(pcoord.y>=size.y-1)
				val = vec4(.0, -1., .0, .0)*g(float(pcoord.x));
			mins = min(val, mins);
			maxs = max(val, maxs);
			result += xw[i]*yw[j]*val;
		}
...
void main() {
	...
	if(mode == 0) { // init
		...
		if(coord.x<size.x-1 && coord.y<size.y-1 && neg.z>.0) {
			...
			if(neg.z<1.)
				new = max(.0, new);
			//if(coord.x==0||coord.y==0||coord.x==size.x-2||coord.y==size.y-2) new = min(.0, new);
