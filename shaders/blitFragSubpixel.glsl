...
void main() {
	...
	for(int sp = 0; sp<3; ++sp) {
		...
		vec2 pixelCoord = floor(vec2(gl_FragCoord.x, screenSize.y-gl_FragCoord.y))-boundingBox.xy;
		...
		for(int i = range.x; i<range.y; i++) {
			...
			if(newColor!=fontColor) {
				color[sp] = mix(color[sp], fontColor[sp], area);
